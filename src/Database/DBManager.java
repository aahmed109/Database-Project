...
public class DBManager {
    ...
    public List<PurchaseHistory> getbetHistory(String d1, String d2){
        List<PurchaseHistory> phistory = new ArrayList<>();
        ...
        String query = "SELECT SERIAL_NO SLNO, ORDER_ID OID , CUSTOMER_ID CID, USER_NAME UNAME, EMAIL MAIL, PAYMENT_TYPE PTYPE , ADDRESS ADDR, ORDERS.\"DATE\" DATES, " +
                "PRODUCT_ID PID , PRODUCT_NAME PNAME, BRAND BRANDS, \"MODEL\" MOD, \"CATEGORY\" CAT, QUANTITY QUANT, \"COST\" COSTS, TOTAL_COST TCOST FROM DETAILED_HISTORY WHERE DATE BETWEEN TO_DATE(?" +
                ", 'DD-MM=YYYY') AND TO_DATE(?, 'DD-MM-YYYY')";
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            ResultSet resultSet = preparedStatement.executeQuery();
            while (resultSet.next()){
